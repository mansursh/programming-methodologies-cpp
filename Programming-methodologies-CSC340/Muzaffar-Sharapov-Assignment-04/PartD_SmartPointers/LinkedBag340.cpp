...
bool LinkedBag<ItemType>::addEnd340(const ItemType& newEntry) {
    unique_ptr<Node<ItemType>> uniqueNodePtr = make_unique<Node<ItemType>>(); //creating new last node
    uniqueNodePtr->setItem(newEntry);
    uniqueNodePtr->setNext(nullptr); //making Ptr the last node that is to be added
    ...
    while (sharedNodePtr->getNext() != nullptr) //get to next until we reach null
    {
        sharedNodePtr = sharedNodePtr->getNext();
    }
    sharedNodePtr->setNext(move(uniqueNodePtr)); //link last node to new node at the end
